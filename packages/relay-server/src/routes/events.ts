...
import { getAgentByRelayId, verifyApiKey } from '../services/auth';

export async function eventsRouter(server: FastifyInstance) {
  ...
  server.get('/pending/:relay_id', async (request, reply) => {
    ...
    // 1. Verify Agent
    const agent = await getAgentByRelayId(relay_id);
    if (!agent) {
  ...
  server.delete('/events/:relay_id/:event_id', async (request, reply) => {
    const { relay_id, event_id } = request.params as { relay_id: string, event_id: string };
    const authHeader = request.headers.authorization;
    ...
    const result = await db.webhook_events.updateMany({
      where: {
        relay_id: relay_id,
